...
@msig trait TempSensor_i_tcp_tempSensor_TestApi {
   ...
   */
  def check_concrete_output(currentTemp: BuildingControl.Temperature_i => B,
  ...
  def get_currentTemp(): Option[BuildingControl.Temperature_i] = {
    val value: Option[BuildingControl.Temperature_i] = get_currentTemp_payload() match {
      ...
      case Some(v) => halt(s"Unexpected payload on port currentTemp.  Expecting 'BuildingControl.Temperature_i_Payload' but received ${v}")
  ...
  def get_currentTemp_payload(): Option[BuildingControl.Temperature_i_Payload] = {
    return Art.observeOutInfrastructurePort(Arch.BuildingControlDemo_i_Instance_tcp_tempSensor.initialization_api.currentTemp_Id).asInstanceOf[Option[BuildingControl.Temperature_i_Payload]]
  }
