...
Z building_control_PlatformNix_seed(StackFrame caller) {
  building_control_PlatformNix_init(caller);
...
Unit building_control_PlatformNix_initialise(StackFrame caller, Z seed, Option_882048 portOpt) {
  ...
  if (!match_16) {
    ...
    if (match_16) {
      ...
      IS_82ABD8__append((IS_82ABD8) &t_2, sf, building_control_PlatformNix_ids(sf), id);
...
void building_control_PlatformNix_receive(Tuple2_D0E3BB result, StackFrame caller, Option_882048 portOpt) {
  ...
  if (!match_26) {
    ...
    if (match_26) {
      ...
      building_control_SharedMemory_receive((art_DataContent) &t_1, sf, Z__add(building_control_PlatformNix_seed(sf), port_27_17));
...
void building_control_PlatformNix_receiveAsync(Option_02FA6D result, StackFrame caller, Option_882048 portOpt) {
  ...
  if (!match_44) {
    ...
    if (match_44) {
      ...
      building_control_SharedMemory_receiveAsync((Option_8E9F45) &t_1, sf, Z__add(building_control_PlatformNix_seed(sf), port_45_17));
...
B building_control_PlatformNix_sendAsync(StackFrame caller, Z app, Z port, art_DataContent data) {
  ...
  sfUpdateLoc(39);
  B r = building_control_SharedMemory_sendAsync(sf, port, Z__add(building_control_PlatformNix_seed(sf), port), (art_DataContent) data);
  return r;
}
...
Unit building_control_PlatformNix_finalise(StackFrame caller) {
  ...
  sfUpdateLoc(56);
  {
    IS_82ABD8 t_0 = building_control_PlatformNix_ids(sf);
    int8_t t_1 = (building_control_PlatformNix_ids(sf))->size;
