...
object ArtDebug_Ext {
  ...
  def injectPort(bridgeId: Art.BridgeId, port: Art.PortId, data: DataContent): Unit = {
    ...
    if(bridge.ports.dataOuts.elements.map(_.id).contains(port) ||
      ...
      ArtNative.logDebug(Art.logTitle, s"Injecting from port ${Art.ports(port).get.id}")
      ...
      ArtNative.logDebug(Art.logTitle, s"Injecting to port ${Art.ports(port).get.id}")
  ...
  private def registerListener(bridge: Bridge, ports: Seq[UPort], listener: Listener): Unit = {
    for (port <- ports) {
      ...
      ArtNative.logDebug(Art.logTitle, s"Registered callback.  Triggered when port ${port.id} $c $t")
