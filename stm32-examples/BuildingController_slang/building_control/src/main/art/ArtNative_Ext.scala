...
object ArtNative_Ext {
  ...
  def run(): Unit = {
    ...
    for (bridge <- bridges) {
      ...
      logInfo(Art.logTitle, s"Initialized bridge: ${bridge.id}")
    }
    ...
    for (bridge <- bridges) {
      ...
      new Thread(() => {
        ...
        logInfo(Art.logTitle, s"Thread for ${bridge.id} instantiated.")
    ...
    for (bridge <- bridges) {
      ...
      logInfo(Art.logTitle, s"Finalized bridge: ${bridge.id}")
    }
