...
object ArtNix {
  ...
  val connection: MS[Art.PortId, ISZ[(Art.PortId, Art.PortId)]] = {
    ...
    r(Arch.PingPongSystem_i_Instance_p_ping.ping_out.id) = ISZ(
      (IPCPorts.Pong_i_App, Arch.PingPongSystem_i_Instance_p_pong.pong_inn.id)
    )
    r(Arch.PingPongSystem_i_Instance_p_pong.pong_out.id) = ISZ(
      (IPCPorts.Ping_i_App, Arch.PingPongSystem_i_Instance_p_ping.ping_inn.id)
    )
